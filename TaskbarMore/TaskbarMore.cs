...
namespace TaskbarMore
{
    public partial class TaskbarMore : Form
    {
        ...
        {
            OSVERSIONINFOEX version = new OSVERSIONINFOEX { dwOSVersionInfoSize = Marshal.SizeOf(typeof(OSVERSIONINFOEX)) };
            return RtlGetVersion(ref version) == 0 && version.dwMajorVersion == 10 && version.dwBuildNumber >= Windows11Build;
        }
        ...
        {
            if (IsWindows11OrLater() || _layeredSurfaceFailed || _layeredSurfaceEnabled || !IsEmbeddedInTaskbar() || !IsHandleCreated)
            {
                return;
            }
        ...
        private void PaintCompositeContent(PaintEventArgs e)
        {
            ...
            bool tryLayered = embedded && !IsWindows11OrLater() && _layeredSurfaceEnabled && !_layeredSurfaceFailed;
        ...
        {
            Color? adjacentTaskbarColor = IsWindows11OrLater() ? TrySampleAdjacentTaskbarColor() : null;
            if (adjacentTaskbarColor.HasValue)
            {
                g.Clear(adjacentTaskbarColor.Value);
                return;
            }
        ...
        private Color? TrySampleAdjacentTaskbarColor()
        {
            if (!IsEmbeddedInTaskbar() || Shell_TrayWnd == IntPtr.Zero) return null;
            RECT widgetRect = new RECT();
            RECT taskbarRect = new RECT();
            if (!GetWindowRect(Handle, ref widgetRect) || !GetWindowRect(Shell_TrayWnd, ref taskbarRect)) return null;
            int y = Math.Max(taskbarRect.Top, Math.Min(taskbarRect.Bottom - 1, widgetRect.Top + (widgetRect.Bottom - widgetRect.Top) / 2));
            int x = widgetRect.Left > taskbarRect.Left ? widgetRect.Left - 1 : widgetRect.Right;
            if (x < taskbarRect.Left || x >= taskbarRect.Right) return null;
            IntPtr hdc = GetDC(IntPtr.Zero);
            if (hdc == IntPtr.Zero) return null;
            try { return SampleScreenColor(hdc, x, y); }
